# os_sim_gui.py
# 迷你操作系统模拟器：
# - 文件系统：多级目录树；左键选中；右键菜单新建文件/新建文件夹；文件显示字节数
# - 运行文件：派生多个逻辑相关但结构不同的进程（页数不同/工作量不同/角色名不同）
# - 虚存：创建进程时为全部虚页分配SWAP页框
# - 内存：每进程固定分配RAM块(页框)数量（取决于进程大小/页数），仅在自己的块内做局部FIFO调页
# - 完成释放：进程结束释放其RAM固定块 + SWAP
# - 调度：MLFQ
# 运行：python os_sim_gui.py
...
# ----------------------------- 默认进程大小（更小，避免SWAP不足） -----------------------------
DEFAULT_PROC_MEM_KB = 16  # GUI“随机创建进程”默认内存
DEFAULT_PROC_WORK = 60  # 默认工作量
...
class PCB:
    ...
    # 固定分配给该进程的 RAM 块集合，以及局部 FIFO 队列
    ram_pool: List[int] = field(
        default_factory=list
    )  # 该进程固定拥有的 RAM frame_id 列表
    ram_fifo: deque = field(
        default_factory=deque
    )  # 该进程已装入页的 frame_id FIFO 队列
...
class SwapFrame:
    frame_id: int
    owner: Optional[Tuple[int, int]] = None  # (pid, vpn)
...
# ============================= RAM（物理页框）+ 每进程固定块 + 局部FIFO =============================


@dataclass
class RAMFrame:
    frame_id: int
    reserved_pid: Optional[int] = None  # 该帧固定分配给哪个进程（保留关系）
    owner: Optional[Tuple[int, int]] = None  # (pid, vpn) 当前装入
...
class MemoryManager:
    ...
    def page_in_local_fifo(
        ...
        # pool 满：局部 FIFO
        if pcb.ram_fifo:
            victim_fid = pcb.ram_fifo.popleft()
        else:
            victim_fid = pcb.ram_pool[0]
...
class MiniOS:
    ...
    def run_file_spawn_processes(
        ...
        for i in range(nprocs):
            delta = int(round(i - center))  # -1,0,+1...
            raw_pages_list.append(max(1, base_pages + delta))

        swap_free = self.swap.free_count()
        pages_list = self._fit_pages_list_to_swap(raw_pages_list, swap_free)
        ...
        if len(pages_list) < len(raw_pages_list):
            self.log(
                f"SWAP不足自动降级：原计划派生{len(raw_pages_list)}个(总页={sum(raw_pages_list)})，"
                f"实际派生{len(pages_list)}个(总页={sum(pages_list)})"
            )
        ...
        self.log(
            f"运行文件：{file_path} -> 派生 {len(created)} 个相关进程（页数/工作量不同；RAM固定块不同）"
        )
        return created
    ...
    def tick(self) -> None:
        ...
        if pcb.slice_left <= 0:
            ...
            self.log(
                f"时间片耗尽：PID={pcb.pid} -> READY，降级/入队Q{pcb.queue_level} slice重置={pcb.slice_left}"
            )
...
class MiniOSGUI:
    ...
    def _build_proc_page(self):
        ...
        widths = [70, 260, 90, 50, 60, 60, 70, 70, 70, 70, 60, 80]
        for c, w in zip(cols, widths):
            self.tree_proc.heading(c, text=c.upper())
        ...
        box_new = ttk.LabelFrame(right, text="创建随机进程（默认更小，避免SWAP不足）")
        ...
        box_runfile = ttk.LabelFrame(
            right, text="运行文件：派生多进程（自动适配SWAP，不会失败）"
        )
        ...
        ttk.Button(box_kill, text="终止选中PID", command=self.on_kill_selected).pack(
    ...
    def _build_mem_page(self):
        ...
        tip = ttk.Label(
            top, text="（浅色=进程保留块空闲；深色=已装入页；完成后该进程颜色块释放）"
        )
        tip.pack(side="left", padx=18)

        ram_box = ttk.LabelFrame(
            frm, text="RAM 物理页框（每进程固定块；仅在自己块内局部FIFO置换）"
        )
        ...
        swap_box = ttk.LabelFrame(frm, text="SWAP 虚存页框（创建进程时先分配全页）")
    ...
    def on_run_file_spawn(self):
        try:
            ...
            # 这里也默认不太大：如果你想更小，就直接改 spin_pmem 或把下面min再调低
