...
class OSDesktop(tk.Tk):
    ...
    def __init__(self):
        ...
        self.launch_mfq = AppLauncher(
            self, "多级反馈队列调度", lambda root: MFQApp(root)
        )
        self.launch_pager = AppLauncher(
            self, "基本分页管理", lambda root: PageApp(root)
        )
        self.launch_pv = AppLauncher(self, "生产者-消费者 PV", lambda root: PVApp(root))
        vm_class = _load_virtual_memory_app()
        self.launch_vm = AppLauncher(
            self, "请求分页虚拟存储器", lambda root: vm_class(root)
        )
    ...
    def _build_layout(self):
        ...
        ttk.Label(header, text="OS2 可视化操作系统", style="Title.TLabel").pack(
            anchor="w"
        )
        ttk.Label(
            header,
        ...
        ttk.Button(
            actions, text="全部启动", style="Accent.TButton", command=self._open_all
    ...
    def _make_overview_tab(self, parent):
        ...
        ttk.Label(tab, text=intro, style="Subtitle.TLabel", justify="left").pack(
            anchor="w"
        )
        ...
        self._render_card(
            cards,
            "请求分页/置换",
            "固定分配 FIFO，内存/磁盘双视图，自动生命周期演示",
            self.launch_vm.open,
        )
        self._render_card(
            cards,
            "生产者-消费者",
            "empty/full/mutex 信号量联动，实时缓冲区动画",
            self.launch_pv.open,
        )
    ...
    def _make_scheduler_tab(self, parent):
        tab = ttk.Frame(parent, style="Card.TFrame", padding=18)
        ttk.Label(tab, text="多级反馈队列调度", style="Title.TLabel").pack(
            anchor="w", pady=(0, 4)
        )
        ttk.Label(
            tab,
    ...
    def _make_memory_tab(self, parent):
        tab = ttk.Frame(parent, style="Card.TFrame", padding=18)
        ttk.Label(tab, text="基本分页存储管理", style="Title.TLabel").pack(
            anchor="w", pady=(0, 4)
        )
        ttk.Label(
            tab,
    ...
    def _make_virtual_tab(self, parent):
        tab = ttk.Frame(parent, style="Card.TFrame", padding=18)
        ttk.Label(tab, text="请求分页虚拟存储器", style="Title.TLabel").pack(
            anchor="w", pady=(0, 4)
        )
        ttk.Label(
            tab,
    ...
    def _make_sync_tab(self, parent):
        tab = ttk.Frame(parent, style="Card.TFrame", padding=18)
        ttk.Label(tab, text="生产者-消费者 (PV)", style="Title.TLabel").pack(
            anchor="w", pady=(0, 4)
        )
        ttk.Label(
            tab,
    ...
    def _render_card(self, parent, title, desc, command):
        ...
        ttk.Button(card, text="打开", style="Accent.TButton", command=command).pack(
            anchor="w"
        )
